app.controller('PlayersCtrl', ['$scope', 'PlayersService', 'StatsService', function ($scope, PlayersService, StatsService) {
    ...
    function calculatePlayerRanking (players){
    	for (p in players) {
    ...
    PlayersService.get().then(function (response) {
        ...
        StatsService.get().then(function (response) {
	        ...
	        for (p in $scope.players){
	        	var temp = {playerId: $scope.players[p].playerId, name: $scope.players[p].name, img:"", wins: 0, losses: 0, points: 0, overallRanking: 0, ranking: 0};
